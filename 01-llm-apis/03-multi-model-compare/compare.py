"""
Lab 03 — Multi-Model Compare

Send the same prompt to multiple local models in parallel.
Results print side by side so you can compare quality and speed.

Run:
  python compare.py "What is a transformer?"
  python compare.py "Write a Python function to reverse a string" --models llama3.3:70b qwen2.5-coder:32b phi4
  python compare.py "Solve: if 2x + 3 = 11, what is x?" --models deepseek-r1 llama3.3:70b phi4
"""

import argparse
import json
import sys
import time
from concurrent.futures import ThreadPoolExecutor, as_completed

import requests
from rich.console import Console
from rich.panel import Panel
from rich.table import Table

OLLAMA_URL = "http://localhost:11434"

DEFAULT_MODELS = [
    "llama3.3:70b",
    "deepseek-r1",
    "phi4",
]

console = Console()


def query_model(model: str, prompt: str) -> dict:
    start = time.perf_counter()
    try:
        resp = requests.post(
            f"{OLLAMA_URL}/api/chat",
            json={
                "model": model,
                "messages": [{"role": "user", "content": prompt}],
                "stream": False,
            },
            timeout=180,
        )
        resp.raise_for_status()
        data = resp.json()
        elapsed = time.perf_counter() - start
        return {
            "model": model,
            "answer": data["message"]["content"],
            "prompt_tokens": data.get("prompt_eval_count", 0),
            "output_tokens": data.get("eval_count", 0),
            "latency_ms": elapsed * 1000,
            "error": None,
        }
    except Exception as exc:
        return {
            "model": model,
            "answer": "",
            "prompt_tokens": 0,
            "output_tokens": 0,
            "latency_ms": (time.perf_counter() - start) * 1000,
            "error": str(exc),
        }


def run_compare(prompt: str, models: list[str]) -> list[dict]:
    console.print(f"\n[bold]Prompt:[/bold] {prompt}")
    console.print(f"[dim]Querying {len(models)} models in parallel...[/dim]\n")

    results = []
    with ThreadPoolExecutor(max_workers=len(models)) as pool:
        futures = {pool.submit(query_model, m, prompt): m for m in models}
        for future in as_completed(futures):
            result = future.result()
            status = "[red]ERROR[/red]" if result["error"] else "[green]done[/green]"
            console.print(f"  {result['model']:30s} {status}  ({result['latency_ms']:.0f} ms)")
            results.append(result)

    return sorted(results, key=lambda r: r["latency_ms"])


def display_results(results: list[dict]):
    console.print()

    # individual panels
    for r in results:
        if r["error"]:
            console.print(Panel(
                f"[red]{r['error']}[/red]",
                title=f"[bold red]{r['model']}[/bold red]",
                border_style="red",
            ))
        else:
            console.print(Panel(
                r["answer"].strip(),
                title=f"[bold cyan]{r['model']}[/bold cyan]  "
                      f"[dim]{r['latency_ms']:.0f} ms · {r['output_tokens']} tokens[/dim]",
                border_style="cyan",
            ))

    # summary table
    table = Table(title="Summary", show_header=True, header_style="bold")
    table.add_column("Model", style="cyan")
    table.add_column("Latency (ms)", justify="right")
    table.add_column("Prompt tokens", justify="right")
    table.add_column("Output tokens", justify="right")
    table.add_column("tok/s", justify="right")
    table.add_column("Status")

    for r in results:
        tps = f"{r['output_tokens'] / (r['latency_ms'] / 1000):.1f}" if r["latency_ms"] > 0 else "—"
        status = "[red]error[/red]" if r["error"] else "[green]ok[/green]"
        table.add_row(
            r["model"],
            f"{r['latency_ms']:.0f}",
            str(r["prompt_tokens"]),
            str(r["output_tokens"]),
            tps,
            status,
        )

    console.print(table)


def main():
    parser = argparse.ArgumentParser(description="Compare multiple Ollama models on one prompt")
    parser.add_argument("prompt", help="The prompt to send to all models")
    parser.add_argument(
        "--models", nargs="+", default=DEFAULT_MODELS,
        help=f"Models to compare (default: {DEFAULT_MODELS})",
    )
    args = parser.parse_args()

    results = run_compare(args.prompt, args.models)
    display_results(results)


if __name__ == "__main__":
    main()
